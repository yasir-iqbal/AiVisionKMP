...
import com.apps.aivisioncmp.database.GetLastConversationId
import com.apps.aivisioncmp.database.GetLastMessageId
...
class LocalDataSource(private val db: AppDatabaseHelper) {
    ...
    suspend fun addMessage(chatMessage: ChatMessage) = db.visionDAO.insertMessage(chatMessage.recentChatId,chatMessage.role,chatMessage.type,chatMessage.content,chatMessage.url,chatMessage.status.toLong(),"${Clock.System.now().toEpochMilliseconds()}")
    ...
    suspend fun getLastConversationId(): GetLastConversationId = db.visionDAO.getLastConversationId().executeAsOne()
    suspend fun getLastMessageId(): GetLastMessageId = db.visionDAO.getLastMessageId().executeAsOne()
